...
namespace ConsoleApp5
{
    class Program
    {
        ...
        public static List<Transaction> Trade(Country country, List<Country> tradeNeighbors,  List<Balance> balanceCountry, IEnumerable<Coin> tradeCoins, int countSendCoins, DateTime currentDate)
        {
            ...
            foreach (var balance in balanceCountry.Where(x => tradeCoins.Contains(x.Coin)))
            {
                ...
                if ((balance.Count + countSendCoins) > sum / 2)
                {
                    balance.Count = sum / 2 - countSendCoins;
                }
        ...
        public static List<Balance> GetBalanceLine(CountryContext db, int countryId, DateTime date, bool xLine)
        {
            ...
            var start = xLine ? (countryId - (countryId % 10)) : ((countryId / 10) * 10);//TODO: its ok???
            var end = start + (xLine ? 9 : 90);
            var step = xLine ? 1 : 10;

            //TODO: refactoring to one query for all countries
            for (var i = start; i <= end; i += step)
            {
                ...
                balanceList.AddRange(GetBalance(db,i, date));
            }

            return balanceList.GroupBy(x => x.Coin).Select(x => new Balance()
            {
                Coin = x.Key,
        ...
        private static void CreateWord(CountryContext db)
        {
            var countries = new List<Country>();

            for (var i = 0; i < 100; i++)
            {
                countries.Add(new Country() {Id = i, Name = i.ToString()});
            }
